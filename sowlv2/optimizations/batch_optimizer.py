...
class IntelligentBatchOptimizer:
    ...
    def profile_and_optimize(self, 
        ...
        detection_batch_size = max(1, int(available_memory * 0.3 / detection_memory_per_batch))
        
        # Segmentation: SAM2 needs ~4GB for base model + more for processing
        segmentation_memory_per_image = 4.0 + base_memory_per_image * 2
        segmentation_batch_size = max(1, int(available_memory * 0.4 / segmentation_memory_per_image))
            ...
            try:
                ...
                if torch.cuda.is_available():
                    ...
                    if memory_used < 0.7:  # Less than 70% memory used
                        current_batch_size = min(current_batch_size + 1, initial_batch_size * 2)
